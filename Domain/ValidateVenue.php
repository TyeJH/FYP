    ...
    if ($nul == 'Update') {
        ...
        if (!empty($namedb)) {
            if ($name != $namedb->venueName) {
             ...
             echo '<script>alert("Venue does not exist.");location.href = "../UI/VenueList.php";</script>';
        }
    } else {
        if ($venda->checkName($name)) {
            $venue = new Venue($id, $name, $desc, $status);
            ...
            echo '<script>alert("Insert Failed. Venue name duplicate.");location.href = "../UI/VenueList.php";</script>';
        }
    }
}
